...
void Menu();
bool UnirNodos(list<Nodo> &Memoria, int &sim);
    ...
    do{
        ...
        case 1:
            ...
            while(Simulacion==false){
                ...
                if(Simulacion==true)
                    break;
                Simulacion=UnirNodos(Memoria,tSim);
                if(Simulacion==true)
                    break;
                cout<<endl<<endl;
            }
...
bool UnirNodos(list<Nodo> &Memoria, int &sim)
{
    ...
    if(sim==0)
        return true;
    for(list<Nodo>::iterator it=Memoria.begin();it!=Memoria.end();++it){ //recorremos la lista
        list<Nodo>::iterator it2=it;									//Creamos un iterador en la posicion siguiente a it
        ++it2;

        if(it->cuanto==0 && it2->cuanto==0)
            sim--;

        if(it->cuanto==0 && it2->cuanto==0){
            cout<<"Uniendo nodos: ";
            cout<<"["<<it->id<<"|"<<it->tam<<"|"<<it->cuanto<<"] y ";
            cout<<"["<<it2->id<<"|"<<it2->tam<<"|"<<it2->cuanto<<"]"<<endl;
            it->tam=it->tam + it2->tam;
            Memoria.erase(it2);
            --it;
